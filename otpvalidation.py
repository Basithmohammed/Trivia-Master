...
<head>
  ...
  <div>
     <form method="post">
    <input type="number" name="ootp" placeholder="Enter OTP">
    <input type="submit" name="sub" value="VERIFY">

</form>
  </div>
  <p class="class"><a href="forgotpassword.py" style="color:white;text-decoration:none;opacity:0.5;">
...
qotp=f.getvalue("ootp")
psub=f.getvalue("sub")
if psub!=None:
    if potp==qotp:
        print("""
            <script>
                location.href="newpassword.py?id=%s";
            </script>
        """%(id))
    else:
        print("""
